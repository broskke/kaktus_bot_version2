...
def get_discription(news_link: str) -> str:
    html = get_html(news_link)
    soup = BeautifulSoup(html, 'html.parser')
    return soup.find('div',class_='BbCode').find('p').text

# print(get_discription('https://kaktus.media/doc/474566_v_2022_gody_kompaniia_intek_vyshla_na_rekordnyu_pribyl_za_poslednie_6_let.html'))


def parse_data(cards: ResultSet) -> list:
    news = []
    for card in cards:
        news_link = card.find('a',class_='ArticleItem--name').get('href')
        obj = {
            'title':card.find('a',class_='ArticleItem--name').text.lstrip('\n'),
            'photo':card.find('img',class_='ArticleItem--image-img').get('src'),
            'news_link': news_link,
            'description':get_discription(news_link),
